...
public class OwnerController {
  ...
  @GetMapping("/{ownerId}")
  public ModelAndView findOwners(@PathVariable String ownerId) {
    ModelAndView view = new ModelAndView("owners/ownerDetails");
    view.addObject(this.ownerService.findById(Long.valueOf(ownerId)));
    return view;
  }
  ...
  public String processUpdateOwnerForm(
      @Valid Owner owner, BindingResult result, @PathVariable Long ownerId) {
    if (result.hasErrors()) {
      return VIEWS_OWNER_CREATE_OR_UPDATE_FORM;
    } else {
      owner.setId(ownerId);
      Owner savedOwner = ownerService.save(owner);
      System.out.println("HELP HERE: " + savedOwner);
      return "redirect:/owners/" + savedOwner.getId();
    }
  }
}
